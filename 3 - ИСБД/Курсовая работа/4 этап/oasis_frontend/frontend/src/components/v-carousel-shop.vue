<template>
  <div class='wrapper'>
    <div class="v-carousel" :style="{'margin-left': '-'+ (100*currentSlideIndex) + '%'}">
      <v-carousel-item-shop
        v-for="item in carousel_data"
        :item_data="item"
      ></v-carousel-item-shop>
    </div>

    <svg class="previous_next_button" @click="prevSlide" style="left: 0px" aria-hidden="true" width="60.08"
         height="60.08"
    ...
    <svg class="previous_next_button" @click="nextSlide" style="left: 1285px" aria-hidden="true" width="60.08"
         height="60.08"
...
<script>
import vCarouselItemShop from './v-carousel-item-shop'

export default {

  name: "v-carousel-shop",
  components: {
    vCarouselItemShop
  },
  props: {
  ...
  watch: {
    interval_new() {
      if (this.interval_new !== 0 && this.interval_new % 9 === 0) {
        this.nextSlide();
      }
    },
